...
if(!$id_warga) {
	...
	$no_ktp = (isset($result['no_ktp'])) ? trim($result['no_ktp']) : '';
...
if(isset($_POST['submit'])):	
	...
	if(empty($errors)):
		...
		$prepare_query = 'UPDATE warga SET no_ktp = :no_ktp, nama = :nama, tanggal_input = :tanggal_input WHERE id_warga = :id_warga';
		$data = array(
			'no_ktp' => $no_ktp,
			'nama' => $nama,
		...
		redirect_to('show-warga.php?status=sukses-edit');
		...
		<div class="main-content the-content">
			...
			<?php else: ?>				
				...
				<form action="edit-warga.php?id=<?php echo $id_warga; ?>" method="post">
					...
					<div class="field-wrap clearfix">
						<button type="submit" name="submit" value="submit" class="button">Simpan warga</button>
					</div>
				</form>
