 ...
 * ------------------------------------------- */
$query2 = $pdo->prepare('SELECT id_warga, no_ktp, nama FROM warga');
$query2->execute();			
...
foreach($matriks_x as $id_kriteria => $nilai_wargas):
	...
	$jumlah_kuadrat = 0;
	foreach($nilai_wargas as $nilai_warga):
		$jumlah_kuadrat += pow($nilai_warga, 2);
	endforeach;
	$akar_kuadrat = sqrt($jumlah_kuadrat);
	
	// Mencari hasil bagi akar kuadrat
	// Lalu dimasukkan ke array $matriks_r
	foreach($nilai_wargas as $id_warga => $nilai_warga):
		$matriks_r[$id_kriteria][$id_warga] = $nilai_warga / $akar_kuadrat;
	endforeach;
...
foreach($kriterias as $kriteria):
	foreach($wargas as $warga):
		...
		$matriks_y[$id_kriteria][$id_warga] = $bobot * $nilai_r;
...
foreach($wargas as $warga):
	...
	foreach($matriks_y as $id_kriteria => $nilai_wargas):
		
		$hsl_pengurangan_jip = $nilai_wargas[$id_warga] - $solusi_ideal_positif[$id_kriteria];
		$hsl_pengurangan_jin = $nilai_wargas[$id_warga] - $solusi_ideal_negatif[$id_kriteria];
	...
	$jarak_ideal_positif[$id_warga] = $akar_kuadrat_jip;
	$jarak_ideal_negatif[$id_warga] = $akar_kuadrat_jin;
...
foreach($wargas as $warga):
	...
	$s_negatif = $jarak_ideal_negatif[$warga['id_warga']];
	$s_positif = $jarak_ideal_positif[$warga['id_warga']];	
	
	$nilai_v = $s_negatif / ($s_positif + $s_negatif);
	
	$ranks[$warga['id_warga']]['id_warga'] = $warga['id_warga'];
	$ranks[$warga['id_warga']]['no_ktp'] = $warga['no_ktp'];
	$ranks[$warga['id_warga']]['nama'] = $warga['nama'];
	$ranks[$warga['id_warga']]['nilai'] = $nilai_v;
...
<div class="container clearfix">	
	...
	<div class="main-content main-content-full the-content">
				...
				<?php foreach($wargas as $warga): ?>
					<tr>
						...
						foreach($kriterias as $kriteria):
							...
							$id_kriteria = $kriteria['id_kriteria'];
							echo '<td>';
							echo round($matriks_r[$id_kriteria][$id_warga], $digit);
							echo '</td>';
						endforeach;
						?>
					</tr>
				...
				<?php foreach($wargas as $warga): ?>
					<tr>
						...
						foreach($kriterias as $kriteria):
							...
							$id_kriteria = $kriteria['id_kriteria'];
							echo '<td>';
							echo round($matriks_y[$id_kriteria][$id_warga], $digit);
							echo '</td>';
						endforeach;
						?>
					</tr>
		...
		<table class="pure-table pure-table-striped">
			<thead>					
				<tr>
					<th class="super-top-left">No. warga</th>
				...
				<?php foreach($wargas as $warga ): ?>
					<tr>
						...
						<td>
							<?php								
							$id_warga = $warga['id_warga'];
							echo round($jarak_ideal_positif[$id_warga], $digit);
							?>
						</td>						
					</tr>
				<?php endforeach; ?>
				...
				<?php foreach($wargas as $warga ): ?>
					<tr>
						...
						<td>
							<?php								
							$id_warga = $warga['id_warga'];
							echo round($jarak_ideal_negatif[$id_warga], $digit);
							?>
						</td>						
					</tr>
				<?php endforeach; ?>
		...
		if(function_exists('array_multisort')):
			foreach ($sorted_ranks as $key => $row) {
				$no_ktp[$key]  = $row['no_ktp'];
				$nilai[$key] = $row['nilai'];
			}
			array_multisort($nilai, SORT_DESC, $no_ktp, SORT_ASC, $sorted_ranks);
		endif;
		?>		
		...
		<table class="pure-table pure-table-striped">
			<thead>					
				<tr>
					<th class="super-top-left">Rangking</th>
					<th>No. warga</th>
					<th>Nama</th>
					<th>Nilai</th>
				</tr>
